...
module.exports = async (opts, type) => {
  ...
  if (type === 'dir') {
    for (const input of inputs) {
      glyph = await getGlyph(input, opts.optimize, svgo.optimize, opts.svgo);
    ...
    for (const input of inputs) {
      ...
      if (input.d) {
        ...
        input.d = opts.optimize ? svgo.optimize(input.d) : input.d;
      } else if (input.path) {
        const { d, width, height, viewBox } = await getGlyph(
          input.path,
          opts.optimize,
          svgo.optimize,
          opts.svgo
  ...
  opts.svg = await ejs.renderFile(_.absPath(opts.templates.font), opts, {
    async: true,
  });
