const { extname, basename, join, resolve, dirname, sep } = require('path');
const fs = require('fs');
const os = require('os');
...
_.clean = (obj) => {
  const n = {};
  Object.keys(obj)
    .filter((k) => obj[k] !== undefined && (!_.isArray(obj[k]) || obj[k].length !== 0))
    .forEach((k) => {
      n[k] = obj[k];
    });
  return n;
...
_.absPath = (obj) => {
  obj = _.cleanPath(obj);
  return !_.isAbsolute(obj) ? resolve(_.root, obj) : obj;
};
